    ...
    .then(function (res) {
      ...
      chain.on("data", (data) => {
        const xmlString = data.toString(),
          ...
          pmId = _.get(doc, "PubmedArticleSet.PubmedArticle.MedlineCitation.PMID", ""),
          filename = pmId ? `pubmed-${pmId}.xml` : `pubmed-undefined${pmIdNotFound++}.xml`;
      ...
      chain.on("end", function () {
        ...
        utils.progress.increment(parseInt(opts.retmax));
        if (!finished) {
